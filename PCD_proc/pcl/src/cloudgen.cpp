...
void loadPCD (NpyArray vert_arr, NpyArray sync_arr, Mat confimg, PointCloud& pointcloud)
{
   ...
   for (int i = 0; i < confimg.rows; i++)
   {
     for (int j = 0; j < confimg.cols; j++)
     {
      ...
      if (confidence>254 && vert[3*k+2]<DEPTH_THRESHOLD && vert[3*k]<X_THRESHOLD && vert[3*k+1]<Y_THRESHOLD){
...
{
   p = new pcl::visualization::PCLVisualizer (argc, argv, "Raw PCD");
   ...
   p->addPointCloud<PointT> (pcloud.makeShared(), "targ", vp_1);
   
   ostringstream pngfile;
   pngfile<<svg<<argv[1]<<"/"<<argv[2]<<"/pics_pcd/"<<k<<".png";
